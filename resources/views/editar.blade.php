            ...
            <div class="card-body text-dark card-custom-p">
                ...
                <form action="{{ route('datosNego.update',$dato->idnegocio) }}" method="post" class="formularioEditar">
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control" type="text" name="nombre" required="" minlength="1" maxlength="30" id="nombre" autocomplete="" required value="{{$dato->nombre}}">
                    </div>

                    <!--pattern="[A-Z][A-Z,a-z,0-9, ,ñ,á,í,é,ó,ú]+"  minlength="10">-->
                    <div class="mb-3">
                        <span>Dirección</span>
                        <input class="form-control" type="text" name="direccion" required="" pattern="{1,50}" maxlength="50" id="direccion" autocomplete="address-level1" required value="{{$dato->direccion}}" onkeypress="return ( (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || (event.charCode == 32 || event.charCode == 44 || event.charCode == 46) )">
                    </div>                                                            

                    <div class="mb-3">
                        <span>Horario de atención/apertura</span>
                        <input class="form-control" type="time" name="horario1" required="" id="horario1" autocomplete="" required value="{{$dato->horarioinicio}}">
                    </div>

                    <div class="mb-3">
                        <span>Horario de atención/cierre</span>
                        <input class="form-control" type="time" name="horario2" required="" id="horario2" autocomplete="" required value="{{$dato->horariofin}}">
                    </div>
                    ...
                    <div class="mb-3">
                        <span>Teléfono</span>
                        <input class="form-control" type="tel" name="telefono" id="telefono" required="" pattern="^[6|7]\d{7}$" minlength="8" maxlength="8" autocomplete="" required value="{{$dato->telefono}}" onkeypress="return (event.charCode >= 48 && event.charCode <= 57)" >
                    </div>
